#based on the pdftitle library: https://github.com/metebalci/pdftitle
...
class TextOnlyDevice(PDFDevice):
    ...
    def recover_last_paragraph(self):
        if self.current_block is None:
            pass #changed to pass and disregard exception, to see what will happen
...
def get_title_from_io(pdf_io, pagenumber_for_title, algo='original', eliot_tfs=[0,1]):
    ...
    if doc.is_extractable:
        ...
        for page in PDFPage.create_pages(doc):
            ...
            if current_page_number == pagenumber_for_title: #PAGE_NUMBER:
...
def extract_title(raw_file):
            #with open(os.path.join(dirname, filename), 'rb') as raw_file:
                try:                 
                    title = get_title_from_io(raw_file, pagenumber_for_title=1)
                    #if title is empty or less than 10 characters change algorithm
                    if title == '' or len(title) <10:
                        title = get_title_from_io(raw_file, pagenumber_for_title=1, algo="eliot") 
                        #comments.append('First page successful, with fontsize list')
                    #else:
                        #comments.append('First page successful')
                    #pdf_titles.append(title)                 
                    #print('First page title: ', title)
                    return title
                except:
                    #print('First page failed')
                    #comments.append('First page failed')
                    try:
                        title = get_title_from_io(raw_file, pagenumber_for_title=2)
                        if title == '' or len(title) <10:
                            title = get_title_from_io(raw_file, pagenumber_for_title=1, algo="eliot")   
                            #comments.append('Second page successful, with fontsize list')
                        #else:
                           #comments.append('Second page successful')                         
                        #pdf_titles.append(title)
                        #print('Second page title: ', title)
                        return title
                    except:
                        #comments.append('Second page failed, using filename')
